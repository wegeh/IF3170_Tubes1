...
class SidewaysMoveHCPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        label = ttk.Label(scrollable_frame, text="Sideways Move HC Algorithm", font=("Helvetica", 16))
        label.pack(pady=20)
        
        self.max_sideways_move_label = tk.Label(scrollable_frame, text="Max Sideways Move:")
        self.max_sideways_move_label.pack(pady=5)
        self.max_sideways_move_entry = tk.Entry(scrollable_frame)
        self.max_sideways_move_entry.pack(pady=5)

        start_button = ttk.Button(scrollable_frame, text="Start", command=self.start_sideways_move)
    ...
    def start_sideways_move(self):
        ...
        try:
            max_sideways = int(self.max_sideways_move_entry.get())
        except ValueError:
            self.result_text.insert(tk.END, "Invalid input for Max Restarts. Please enter an integer.\n")
            return
        self.result_text.delete(1.0, tk.END)
        self.result_text.insert(tk.END, "Running Sideways Move Hill Climbing...\n")
        self.algorithm_running = True
        self.initial_3d_button.config(state="disabled")
        self.final_3d_button.config(state="disabled")
        self.result_queue = Queue()
        thread = Thread(target=self.run_algorithm, args=(max_sideways, self.result_queue))
        thread.start()
