...
class RandomRestartHCPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        label = ttk.Label(scrollable_frame, text="Random Restart HC Algorithm", font=("Helvetica", 16))
        label.pack(pady=20)
        
        self.max_restart_label = tk.Label(scrollable_frame, text="Max Restarts:")
        self.max_restart_label.pack(pady=5)
        self.max_restart_entry = tk.Entry(scrollable_frame)
        self.max_restart_entry.pack(pady=5)

        start_button = ttk.Button(scrollable_frame, text="Start", command=self.start_random_restart)
    ...
    def start_random_restart(self):
        self.result_text.delete(1.0, tk.END)
        try:
            max_restart = int(self.max_restart_entry.get())
        except ValueError:
            self.result_text.insert(tk.END, "Invalid input for Max Restarts. Please enter an integer.\n")
            return
        self.result_text.delete(1.0, tk.END)
        self.result_text.insert(tk.END, "Running Random Restart Hill Climbing...\n")
        self.algorithm_running = True
        self.initial_3d_button.config(state="disabled")
        self.final_3d_button.config(state="disabled")
        self.result_queue = Queue()
        thread = Thread(target=self.run_algorithm, args=(max_restart, self.result_queue))
        thread.start()
