...
from StochasticHC import StochasticHillClimbing
...
class StochasticHCPage(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        label = ttk.Label(scrollable_frame, text="Stochastic HC Algorithm", font=("Helvetica", 16))
        label.pack(pady=20)

        start_button = ttk.Button(scrollable_frame, text="Start", command=self.start_stochastic)
    ...
    def start_stochastic(self):
        self.result_text.delete(1.0, tk.END)
        self.result_text.insert(tk.END, "Running Stochastic HC Algorithm...\n")

        self.result_queue = Queue()
        thread = Thread(target=self.run_algorithm, args=(self.result_queue,))
        thread.start()

        self.after(100, self.check_queue)

    def run_algorithm(self, result_queue):
        cube = Cube(5)
        algorithm = StochasticHillClimbing(cube)
